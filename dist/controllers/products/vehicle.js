...
router.post('/new', upload.array('files[]'), async (req, res) => {
    ...
    consume = await multipleUpload.then(data => {
        product.new(req.user.person_id, null, description, 'vehicle', null, location).then(async new_product => {
            ...
            vehicle.new(product_id, brand, model, distance, year, fuel, negotiable, finance, interior, unique_owner, post_time, windows, steer, ac).then(data => {
                res.send({ 
                    status: 200 
                })
            }).catch(err => {
