...
module.exports.all = () =>{
    return new Promise((res,rej)=>{
        db.connect().then(obj=>{
            obj.any('select person.name, product.description, service.service_id from person inner join product on person.person_id = product.user_id inner join service on product.product_id = service.product_id').then(data=>{
                res(data);
                obj.done();
            }).catch(error=>{
                rej(error);
